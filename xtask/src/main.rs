...
use crate::utils::{
    Paths, files_recursively, paths, remove_path, require_success, temporary_configs,
};
...
/// Prints the version used by Tauri bundles and GitHub releases.
fn print_version(paths: &Paths) -> Result<()> {
    println!("{}", stable_version(paths)?);
    Ok(())
}
